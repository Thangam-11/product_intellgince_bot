...
from src.rag_app.api_services.schmeas.chat import ChatRequest
...
async def chat(
    ...
    logger.info("Chat request", extra={"request_id": request_id, "query_length": len(body.msg)})

    result = await invoke_chain(body.msg)
    return JSONResponse({"response": result, "request_id": request_id})
...
async def stream_chat(
    ...
    async def generate():
        try:
            async for chunk in invoke_chain_stream(body.msg):
                yield f"data: {chunk}\n\n"
