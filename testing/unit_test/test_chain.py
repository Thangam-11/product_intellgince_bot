...
async def test_invoke_chain_returns_cached_response():
    ...
    with patch("rag_app.core_app.chain._get_redis") as mock_redis:
...
async def test_invoke_chain_calls_llm_on_cache_miss():
    ...
    with patch("rag_app.core_app.chain._get_redis") as mock_redis, \
         patch("rag_app.core_app.chain.get_chain") as mock_get_chain:
...
def test_cache_key_is_deterministic():
    ...
    from rag_app.core_app.chain import _cache_key
...
def test_get_chain_returns_same_instance():
    ...
    with patch("rag_app.core_app.chain._build_chain") as mock_build:
        mock_build.return_value = MagicMock()
        from rag_app.core_app.chain import get_chain
        import rag_app.core_app.chain as chain_module
